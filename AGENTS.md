...
Prefer graph first; fallback to grep/glob/read for literals/configs/insufficient graph.
...
4. **Pre-Execution Justification**: Before invoking mutating/install/destructive terminal/bash command or script, you MUST first output a brief 1-line text message explaining WHAT the command does and WHY it is necessary for the current step.
